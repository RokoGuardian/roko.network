<template>
    <main class="about">
        <div class="center">
            <p class="copyclip">COPY CONTRACT ADDRESS BY CLICKING ON MEZZ</p>
            <h1 @click="copy" class="title">MEZZ</h1>
            <div class="approw">
                <a class="whitepaper" href="/MEZZ.pdf"> <img style="width:8vw;padding:1vw;margin:auto;" src="~/@/assets/whitepaper.svg" alt="">    <p>WHITEPAPER</p>        </a>
    
                <a href="https://medium.com/@mezzfinance/mezz-a-simplified-explainer-22753ab6ba07"><svg width="8vw" viewBox="0 0 461 461" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" xml:space="preserve" xmlns:serif="http://www.serif.com/" style="fill-rule:evenodd;clip-rule:evenodd;stroke-linecap:round;stroke-linejoin:round;stroke-miterlimit:1.5;">
        <g transform="matrix(1,0,0,1,-1331.89,-1393.19)">
            <circle cx="1562.2" cy="1623.5" r="210.036" style="fill:none;stroke:rgb(0,255,162);stroke-width:40.55px;"/>
            <g transform="matrix(-0.10708,0,0,-0.10708,1729.48,1891.56)">
                <circle cx="1562.2" cy="1623.5" r="210.036" style="fill:rgb(0,255,162);"/>
            </g>
            <g transform="matrix(-0.10708,0,0,-0.10708,1729.48,1807.24)">
                <circle cx="1562.2" cy="1623.5" r="210.036" style="fill:rgb(0,255,162);"/>
            </g>
            <g transform="matrix(-0.10708,0,0,-0.10708,1729.48,1703.12)">
                <circle cx="1562.2" cy="1623.5" r="210.036" style="fill:rgb(0,255,162);"/>
            </g>
            <g transform="matrix(1,0,0,1,0,29.2792)">
                <rect x="1539.71" y="1500" width="44.981" height="104.117" style="fill:rgb(0,255,162);"/>
            </g>
        </g>
    </svg>
     <p>ROADMAP</p> </a>
                <a href="https://app.uniswap.org/#/swap?outputCurrency=0xc4c346edc55504574cceb00aa1091d22404a4bc3"><svg width="8vw" fill="rgb(255, 0, 199)" class="Icons__UniLogo-sc-dy67gv-0 iEwsPo" viewBox="0 0 14 15" xmlns="http://www.w3.org/2000/svg"><path d="M4.152 1.551c-.188-.029-.196-.032-.107-.045.17-.026.57.009.846.074.644.152 1.23.542 1.856 1.235l.166.184.238-.038c1.002-.16 2.02-.033 2.873.358.235.108.605.322.65.377.016.018.043.13.06.251.064.418.033.737-.096.976-.07.13-.074.171-.027.283a.274.274 0 0 0 .246.154c.212 0 .44-.34.545-.814l.042-.189.083.094c.457.514.815 1.214.876 1.712l.016.13-.076-.118a1.462 1.462 0 0 0-.435-.453c-.306-.201-.63-.27-1.486-.315-.774-.04-1.212-.106-1.646-.247-.739-.24-1.111-.558-1.989-1.702-.39-.509-.63-.79-.87-1.016-.545-.515-1.08-.785-1.765-.89Z"></path><path d="M10.85 2.686c.019-.34.065-.565.159-.77a.825.825 0 0 1 .077-.148c.005 0-.011.06-.036.133-.068.2-.08.472-.032.789.06.402.093.46.52.894.201.204.434.46.519.571l.154.2-.154-.143c-.188-.175-.62-.517-.716-.566-.064-.032-.074-.032-.113.007-.037.036-.044.09-.05.346-.007.399-.062.655-.194.91-.071.14-.082.11-.018-.047.048-.116.053-.168.053-.554 0-.775-.094-.962-.637-1.28a5.971 5.971 0 0 0-.504-.26 1.912 1.912 0 0 1-.246-.12c.015-.015.545.139.758.22.318.122.37.137.409.123.025-.01.038-.085.05-.305ZM4.517 4.013c-.381-.522-.618-1.323-.566-1.922l.015-.185.087.015c.164.03.445.134.577.214.361.218.518.505.677 1.243.047.216.108.46.136.544.045.133.217.444.356.646.1.146.034.215-.188.195-.339-.03-.798-.345-1.094-.75ZM10.386 7.9c-1.784-.713-2.412-1.333-2.412-2.378 0-.154.005-.28.012-.28.006 0 .075.05.153.113.362.288.767.411 1.889.574.66.096 1.03.173 1.373.286 1.09.359 1.763 1.087 1.924 2.08.046.288.02.828-.057 1.113-.06.225-.242.63-.29.646-.014.005-.027-.046-.03-.116-.018-.372-.208-.735-.526-1.007-.362-.309-.848-.555-2.036-1.03ZM9.134 8.197a3.133 3.133 0 0 0-.086-.375l-.046-.135.085.095c.117.13.21.297.288.52.06.17.066.22.066.496 0 .271-.008.328-.064.48a1.518 1.518 0 0 1-.376.596c-.326.33-.745.512-1.35.588-.105.013-.411.035-.68.049-.679.035-1.126.108-1.527.248a.324.324 0 0 1-.115.027c-.016-.016.258-.178.483-.286.318-.153.635-.236 1.345-.353.35-.058.713-.129.805-.157.868-.264 1.315-.947 1.172-1.793Z"></path><path d="M9.952 9.641c-.237-.506-.292-.995-.162-1.451.014-.05.036-.089.05-.089.013 0 .07.03.124.067.11.073.328.196.912.512.728.395 1.144.7 1.426 1.05.247.305.4.654.474 1.078.042.24.017.82-.045 1.062-.196.764-.65 1.364-1.3 1.714-.095.051-.18.093-.19.093-.009 0 .026-.087.077-.194.219-.454.244-.895.079-1.386-.102-.301-.308-.668-.724-1.289-.484-.72-.602-.913-.721-1.167ZM3.25 12.374c.663-.556 1.486-.95 2.237-1.072a3.51 3.51 0 0 1 1.161.045c.48.122.91.396 1.133.721.218.319.312.596.41 1.214.038.243.08.488.092.543.073.32.216.576.392.704.28.204.764.217 1.239.033a.618.618 0 0 1 .155-.048c.017.017-.222.176-.39.26a1.334 1.334 0 0 1-.648.156c-.435 0-.796-.22-1.098-.668a5.3 5.3 0 0 1-.296-.588c-.318-.721-.475-.94-.844-1.181-.322-.21-.737-.247-1.049-.095-.41.2-.524.72-.23 1.05a.911.911 0 0 0 .512.266.545.545 0 0 0 .619-.544c0-.217-.084-.34-.295-.436-.289-.129-.598.022-.597.291 0 .115.051.187.167.24.074.033.076.035.015.023-.264-.055-.326-.372-.114-.582.256-.252.784-.141.965.204.076.145.085.433.019.607-.15.39-.582.595-1.022.483-.3-.076-.421-.158-.782-.527-.627-.642-.87-.767-1.774-.907l-.174-.027.197-.165Z"></path><path fill-rule="evenodd" clip-rule="evenodd" d="M.308.884C2.402 3.41 3.845 4.452 4.005 4.672c.132.182.082.346-.144.474a1.381 1.381 0 0 1-.515.143c-.147 0-.198-.056-.198-.056-.085-.08-.133-.066-.57-.837A132.96 132.96 0 0 0 1.45 2.67c-.032-.03-.031-.03 1.067 1.923.177.407.035.556.035.614 0 .118-.033.18-.179.343-.244.27-.353.574-.432 1.203-.088.705-.336 1.203-1.024 2.056-.402.499-.468.59-.57.792-.128.253-.163.395-.177.714-.015.339.014.557.118.88.09.284.186.47.429.844.21.323.33.563.33.657 0 .074.014.074.34.001.776-.174 1.407-.48 1.762-.857.22-.233.271-.361.273-.68.001-.208-.006-.252-.063-.372-.092-.195-.26-.358-.63-.61-.486-.33-.694-.595-.75-.96-.048-.3.007-.511.275-1.07.278-.58.347-.827.394-1.41.03-.377.071-.526.18-.646.114-.124.216-.166.498-.204.459-.063.75-.18.99-.4a.853.853 0 0 0 .31-.652l.01-.21-.117-.134C4.098 4.004.026.5 0 .5-.005.5.133.673.308.884Zm.976 9.815a.37.37 0 0 0-.115-.489c-.15-.1-.385-.052-.385.077 0 .04.022.069.072.094.084.043.09.091.024.19-.067.099-.061.186.015.246.123.095.297.043.389-.118ZM4.925 5.999c-.215.065-.424.292-.49.53-.039.145-.016.4.043.478.096.127.188.16.439.159.49-.003.916-.212.966-.474.04-.214-.147-.51-.405-.641a.965.965 0 0 0-.553-.052Zm.574.445c.075-.107.042-.222-.087-.3-.244-.149-.615-.026-.615.204 0 .115.193.24.37.24.118 0 .28-.07.332-.144Z"></path></svg> <p>UNISWAP</p> </a>
                <a href="https://app.sushi.com/swap?inputCurrency=ETH&outputCurrency=0xC4c346eDc55504574cCEB00AA1091d22404A4bC3&chainId=1"><svg width="8vw"  xmlns="http://www.w3.org/2000/svg" class="mr-2 hover:animate-heartbeat" fill="none" viewBox="0 0 990 916"><path fill="url(#paint0_linear_12919_18033)" fill-rule="evenodd" d="M969.311 593.493l-193.11 269.99c-28.12 39.37-82.96 56.71-152.8 51.09-97.03-8.44-225.93-60-349.2-148.58a975.047 975.047 0 01-113.6-94.36c-65.32-63.44-113.33-129.44-139.04-188.29-28.12-64.68-29.05-121.86-.93-161.23L214.2 52.123c28.13-39.37 82.5-56.71 152.81-51.09 97.03 7.97 225.46 60 349.19 148.12 123.28 88.59 213.75 194.05 252.18 283.11 3.32 7.64 6.26 15.17 8.82 22.59 19.11 55.35 16.91 103.92-7.89 138.64z" clip-rule="evenodd"></path><path fill="#fff" fill-rule="evenodd" d="M938.381 445.383c-37.03-84.84-123.75-184.68-241.86-269.05-117.65-84.37-239.98-134.52-332.32-142.02-56.25-4.69-100.78 5.63-123.27 37.03l-.95 1.87c-21.09 31.41-16.4 75.94 5.63 126.56 37.03 85.31 123.74 185.14 241.4 269.51 117.64 84.371 239.98 134.53 332.31 142.03 55.31 4.21 98.91-5.629 121.87-35.159l1.41-2.341c22.5-30.94 17.81-76.87-4.22-128.43zm-172.96 1.88c-10.31 14.52-31.4 18.75-57.18 16.4-46.41-3.75-107.34-29.06-166.4-71.24-59.06-42.19-102.65-91.88-120.93-134.53-10.31-23.9-13.12-44.99-2.81-59.53 10.32-14.53 31.41-18.75 57.65-16.87 45.93 4.22 107.34 29.06 165.93 71.25 59.06 42.18 102.65 92.33 120.93 134.99 10.78 23.9 13.59 44.99 2.81 59.53z" clip-rule="evenodd"></path><defs><linearGradient id="paint0_linear_12919_18033" x1="336.076" x2="653.893" y1="-11.067" y2="926.765" gradientUnits="userSpaceOnUse"><stop stop-color="#27B0E6"></stop><stop offset="0.107" stop-color="#49A1DB"></stop><stop offset="0.288" stop-color="#7D8ACA"></stop><stop offset="0.445" stop-color="#A279BD"></stop><stop offset="0.572" stop-color="#BA6FB6"></stop><stop offset="0.651" stop-color="#C26BB3"></stop><stop offset="0.678" stop-color="#D563AD"></stop><stop offset="0.715" stop-color="#E65BA7"></stop><stop offset="0.76" stop-color="#F156A3"></stop><stop offset="0.824" stop-color="#F853A1"></stop><stop offset="1" stop-color="#FA52A0"></stop></linearGradient></defs></svg> <p>SUSHISWAP</p></a>
                <a href="https://dexscreener.com/ethereum/0x1ad651a07b98464aa42f6cad68d0ce8639b659a5"><svg  fill="#ffff66"  xmlns="http://www.w3.org/2000/svg" width="8vw"  fill-rule="evenodd" viewBox="0 0 252 300" focusable="false" class="chakra-icon custom-euf446"><path d="M151.818 106.866c9.177-4.576 20.854-11.312 32.545-20.541 2.465 5.119 2.735 9.586 1.465 13.193-.9 2.542-2.596 4.753-4.826 6.512-2.415 1.901-5.431 3.285-8.765 4.033-6.326 1.425-13.712.593-20.419-3.197m1.591 46.886l12.148 7.017c-24.804 13.902-31.547 39.716-39.557 64.859-8.009-25.143-14.753-50.957-39.556-64.859l12.148-7.017a5.95 5.95 0 003.84-5.845c-1.113-23.547 5.245-33.96 13.821-40.498 3.076-2.342 6.434-3.518 9.747-3.518s6.671 1.176 9.748 3.518c8.576 6.538 14.934 16.951 13.821 40.498a5.95 5.95 0 003.84 5.845zM126 0c14.042.377 28.119 3.103 40.336 8.406 8.46 3.677 16.354 8.534 23.502 14.342 3.228 2.622 5.886 5.155 8.814 8.071 7.897.273 19.438-8.5 24.796-16.709-9.221 30.23-51.299 65.929-80.43 79.589-.012-.005-.02-.012-.029-.018-5.228-3.992-11.108-5.988-16.989-5.988s-11.76 1.996-16.988 5.988c-.009.005-.017.014-.029.018-29.132-13.66-71.209-49.359-80.43-79.589 5.357 8.209 16.898 16.982 24.795 16.709 2.929-2.915 5.587-5.449 8.814-8.071C69.31 16.94 77.204 12.083 85.664 8.406 97.882 3.103 111.959.377 126 0m-25.818 106.866c-9.176-4.576-20.854-11.312-32.544-20.541-2.465 5.119-2.735 9.586-1.466 13.193.901 2.542 2.597 4.753 4.826 6.512 2.416 1.901 5.432 3.285 8.766 4.033 6.326 1.425 13.711.593 20.418-3.197"></path><path d="M197.167 75.016c6.436-6.495 12.107-13.684 16.667-20.099l2.316 4.359c7.456 14.917 11.33 29.774 11.33 46.494l-.016 26.532.14 13.754c.54 33.766 7.846 67.929 24.396 99.193l-34.627-27.922-24.501 39.759-25.74-24.231L126 299.604l-41.132-66.748-25.739 24.231-24.501-39.759L0 245.25c16.55-31.264 23.856-65.427 24.397-99.193l.14-13.754-.016-26.532c0-16.721 3.873-31.578 11.331-46.494l2.315-4.359c4.56 6.415 10.23 13.603 16.667 20.099l-2.01 4.175c-3.905 8.109-5.198 17.176-2.156 25.799 1.961 5.554 5.54 10.317 10.154 13.953 4.48 3.531 9.782 5.911 15.333 7.161 3.616.814 7.3 1.149 10.96 1.035-.854 4.841-1.227 9.862-1.251 14.978L53.2 160.984l25.206 14.129a41.926 41.926 0 015.734 3.869c20.781 18.658 33.275 73.855 41.861 100.816 8.587-26.961 21.08-82.158 41.862-100.816a41.865 41.865 0 015.734-3.869l25.206-14.129-32.665-18.866c-.024-5.116-.397-10.137-1.251-14.978 3.66.114 7.344-.221 10.96-1.035 5.551-1.25 10.854-3.63 15.333-7.161 4.613-3.636 8.193-8.399 10.153-13.953 3.043-8.623 1.749-17.689-2.155-25.799l-2.01-4.175z"></path></svg> <p>DEXSCREENER</p></a>
                <a href="https://twitter.com/MEZZTOKEN"><svg width="8vw" height="8vw" fill="#1da0f1" viewBox="0 0 24 24" aria-hidden="true" class="r-13v1u17 r-4qtqp9 r-yyyyoo r-16y2uox r-8kz0gk r-dnmrzs r-bnwqim r-1plcrui r-lrvibr r-lrsllp"><g><path d="M23.643 4.937c-.835.37-1.732.62-2.675.733.962-.576 1.7-1.49 2.048-2.578-.9.534-1.897.922-2.958 1.13-.85-.904-2.06-1.47-3.4-1.47-2.572 0-4.658 2.086-4.658 4.66 0 .364.042.718.12 1.06-3.873-.195-7.304-2.05-9.602-4.868-.4.69-.63 1.49-.63 2.342 0 1.616.823 3.043 2.072 3.878-.764-.025-1.482-.234-2.11-.583v.06c0 2.257 1.605 4.14 3.737 4.568-.392.106-.803.162-1.227.162-.3 0-.593-.028-.877-.082.593 1.85 2.313 3.198 4.352 3.234-1.595 1.25-3.604 1.995-5.786 1.995-.376 0-.747-.022-1.112-.065 2.062 1.323 4.51 2.093 7.14 2.093 8.57 0 13.255-7.098 13.255-13.254 0-.2-.005-.402-.014-.602.91-.658 1.7-1.477 2.323-2.41z"></path></g></svg><p>TWITTER</p></a>
                <a href="http://t.me/mezzcommunity"><svg fill="#aaffee" width="8vw" id="Bold" enable-background="new 0 0 24 24" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="m9.417 15.181-.397 5.584c.568 0 .814-.244 1.109-.537l2.663-2.545 5.518 4.041c1.012.564 1.725.267 1.998-.931l3.622-16.972.001-.001c.321-1.496-.541-2.081-1.527-1.714l-21.29 8.151c-1.453.564-1.431 1.374-.247 1.741l5.443 1.693 12.643-7.911c.595-.394 1.136-.176.691.218z"/></svg><p>TELEGRAM</p></a>
                <a href="https://medium.com/@mezzfinance/mezz-an-introduction-to-bidding-backed-collateral-extension-for-defi-c5268721ce04"><svg height="8vw" fill="orange" viewBox="0 0 1043.63 592.71" class="at au"><g data-name="Layer 2"><g data-name="Layer 1"><path d="M588.67 296.36c0 163.67-131.78 296.35-294.33 296.35S0 460 0 296.36 131.78 0 294.34 0s294.33 132.69 294.33 296.36M911.56 296.36c0 154.06-65.89 279-147.17 279s-147.17-124.94-147.17-279 65.88-279 147.16-279 147.17 124.9 147.17 279M1043.63 296.36c0 138-23.17 249.94-51.76 249.94s-51.75-111.91-51.75-249.94 23.17-249.94 51.75-249.94 51.76 111.9 51.76 249.94"></path></g></g></svg>
                                                                                                                                                                                                                                                                                                                                                    <p>MEDIUM</p></a>
            </div>
        </div>
    </main>
</template>

<script>
import useClipboard from 'vue-clipboard3'
export default ({
    setup() {
        const { toClipboard } = useClipboard()
        const copy = async () => {
            try {
                await toClipboard('0xC4c346eDc55504574cCEB00AA1091d22404A4bC3')
                console.log('Copied to clipboard')
            } catch (e) {
                console.error(e)
            }
        }

        return { copy }
    },
})
</script>

<style scoped lang="scss">
@import url('https://fonts.googleapis.com/css2?family=Mochiy+Pop+One&family=Permanent+Marker&display=swap');
.about {
    height: 100vh;
    width: 100vw;
    display: flex;
    flex-flow: wrap;
    justify-content: center;
    margin: auto;
}

.center {
    margin: auto;
}

@keyframes float {
    0% {
        margin-top: 0vw;
    }
    50% {
        margin-top: -1%;
    }
    100% {
        margin-top: 0vw;
    }
}

.copyclip {
    font-family: 'Mochiy Pop One', sans-serif;
    -webkit-background-clip: text;
    -webkit-text-fill-color: #fff9;
    background-image: url(/mezz.gif);
    text-align: center;
    line-height: 2vw;
    height: 0px;
    margin: auto 0px;
    font-size: 2.5vw;
    opacity: 0;
    animation: fade 10s ease 1;
}

.whitepaper {
    font-family: 'Mochiy Pop One', sans-serif;
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-image: url(/mezz.gif);
    text-align: center;
    margin: auto 0px;
    font-size: 2.5vw;
    text-decoration: unset;
}

.approw {
    display: flex;
    max-width: 100%;
    flex-flow: wrap;
    z-index: 5;
    position: relative;
    justify-content: space-between;
    a {
        display: flex;
        flex-flow: column;
        justify-content: center;
        transition: all 0.5s ease;
        height: 8vw;
        position: relative;
        svg {
            transition: all 0.5s ease;
        }
        img {
            transition: all 0.5s ease;
        }
        p {
            font-family: 'Mochiy Pop One', sans-serif;
            font-size: 0.8vw;
            font-weight: bold;
            -webkit-background-clip: text;
            -webkit-text-fill-color: #fff5;
            background-image: url(/mezz.gif);
            background-size: cover;
            position: absolute;
            text-decoration: none;
            transition: all 0.5s ease;
            width: 8vw;
            opacity: 0;
            text-align: center;
            bottom: -2vw;
        }
        &:hover {
            svg {
                transform: scale(1.05);
            }
            img {
                transform: scale(1.05);
            }
            p {
                opacity: 1;
            }
        }
    }
}

@keyframes fade {
    0% {
        opacity: 1;
        display: flex;
    }
    100% {
        opacity: 0;
    }
}

.desc {
    margin-top: 4vw;
    p {
        width: 500px;
        max-width: 80vw;
        color: white;
        font-size: 3vw;
        line-height: 5vw;
    }
}

.title {
    cursor: pointer;
    line-height: 30vw;
    margin: auto;
    -webkit-user-select: none;
    -moz-user-select: none;
    -ms-user-select: none;
    user-select: none;
    text-align: center;
    height: min-content;
    background-size: cover;
    font-size: 25vw;
    transition: all 0.2s ease-out;
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    background-image: url(/mezz.gif);
    position: relative;
    z-index: 2;
    display: flex;
    flex-flow: row;
    justify-content: center;
    overflow: visible;
    &:active {
        text-shadow: 0px 0px 10px white;
    }
    &:before {
        content: 'MEZZ';
        filter: brightness(1.2) saturate(1.7) blur(20px);
        position: absolute;
        left: auto;
        mix-blend-mode: plus-lighter;
        cursor: pointer;
        line-height: 30vw;
        margin: auto;
        -webkit-user-select: none;
        -moz-user-select: none;
        -ms-user-select: none;
        user-select: none;
        text-align: center;
        height: min-content;
        background-size: cover;
        font-size: 25vw;
        transition: all 0.2s ease-out;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        background-image: url(/mezz.gif);
    }
}

@media screen and (max-width:600px) {
    .approw {
        display: flex;
        max-width: 100%;
        flex-flow: wrap;
        justify-content: space-around;
        a {
            display: flex;
            flex-flow: column;
            justify-content: center;
            transition: all 0.5s ease;
            height: 20vw;
            margin: 3vw;
            width: 20vw;
            position: relative;
            svg {
                margin: auto;
                transform: scale(1.2);
                transition: all 0.5s ease;
            }
            img {
                transform: scale(1.2);
            }
            p {
                opacity: 1;
                font-size: 2vw;
                font-weight: bold;
                position: absolute;
                text-decoration: none;
                width: 20vw;
                text-align: center;
                bottom: -2vw;
            }
            &:hover {
                svg {
                    transform: scale(1.5);
                }
                img {
                    transform: scale(1.5);
                }
            }
        }
    }
}

@media screen and (min-width:200vh) {
    .title {
        cursor: pointer;
        line-height: 35vh;
        margin: auto;
        -webkit-user-select: none;
        -moz-user-select: none;
        -ms-user-select: none;
        user-select: none;
        text-align: center;
        height: min-content;
        background-size: cover;
        font-size: 30vh;
        transition: all 0.2s ease-out;
        -webkit-background-clip: text;
        -webkit-text-fill-color: transparent;
        background-image: url(/mezz.gif);
        position: relative;
        z-index: 2;
        display: flex;
        flex-flow: row;
        justify-content: center;
        overflow: visible;
        &:active {
            text-shadow: 0px 0px 10px white;
        }
        &:before {
            content: 'MEZZ';
            filter: brightness(1.2) saturate(1.7) blur(20px);
            position: absolute;
            left: auto;
            mix-blend-mode: plus-lighter;
            cursor: pointer;
            line-height: 35vh;
            margin: auto;
            -webkit-user-select: none;
            -moz-user-select: none;
            -ms-user-select: none;
            user-select: none;
            text-align: center;
            height: min-content;
            background-size: cover;
            font-size: 30vh;
            transition: all 0.2s ease-out;
            -webkit-background-clip: text;
            -webkit-text-fill-color: transparent;
            background-image: url(/mezz.gif);
        }
    }
}
</style>